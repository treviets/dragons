...
public interface RoomRepository extends JpaRepository<Room, Integer> {
	...
	@Query(value = "SELECT room FROM Room room WHERE room.id NOT IN (:ids) "
			+ "AND (room.homeId =:homeId OR :homeId is NULL OR :homeId = 0) "
			+ "AND (room.numberOfGuest >=:numberOfGuest OR :numberOfGuest is NULL OR :numberOfGuest = 0) "
			+ "AND ((room.price >= :min OR :min is NULL) AND (room.price <= :max OR :max is NULL)) "
			+ "AND (room.roomType = :roomtype OR :roomtype = 0 OR :roomtype is NULL)")
